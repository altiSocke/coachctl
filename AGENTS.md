...
```
 ...
 │         athlete.yaml   dist/data.json   .env        │
...
#### Build artifacts (sibling of `profile/`)

| Path | Purpose | Writer |
|---|---|---|
| `dist/data.json` | Baked dashboard payload, read by Vercel at cold-start | `bake` |
| `data/activities.db` | Strava activity cache, queryable via SQL | `sync_activities` |

`dist/` and `data/` are committed to the personal repo (so Vercel and other
clones get them) but contain only generated content — never hand-edit.
...
* `deploy/web.py` calls `coachctl.dashboard.create_app(<repo>/dist/data.json)`.
...
* `bake` writes `dist/data.json`; `git push` of that file triggers Vercel
  redeploy.
