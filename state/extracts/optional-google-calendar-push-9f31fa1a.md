...
DEPLOY_WEB_PY = '''"""Vercel entry point — serves deploy/dist/data.json via FastAPI."""

from pathlib import Path

from coachctl.dashboard import create_app

app = create_app(Path(__file__).parent / "dist" / "data.json")
'''
...
def scaffold(target: Path, label: str | None = None) -> None:
    ...
    (target / "deploy" / "dist").mkdir(parents=True, exist_ok=True)
