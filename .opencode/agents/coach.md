...
> - `wiki/` — LLM-maintained, athlete-agnostic. Tracked in personal repo (`coachctl-personal/wiki/`).
> - `profile/` — LLM-maintained, athlete-specific. Lives in `<DATA_ROOT>/profile/`.
> - Boundary: *if two athletes could share it → general wiki; otherwise personal profile. If unsure, personal wins.*
...
- **Layout:** see `AGENTS.md` (canonical). Paths the coach cares about:
  - `<DATA_ROOT>/wiki/` — LLM-maintained general knowledge (personal repo).
  - `<DATA_ROOT>/wiki/sources/` — paper catalogues synthesised from `raw/` literature.
  - `<DATA_ROOT>/raw/` — immutable source documents (human-write-only, personal repo).
  - `<DATA_ROOT>/profile/` — LLM-maintained personal knowledge (personal repo).
...
4. **Git commit & push:**
   ...
   - **Wiki/raw changes** (general wiki, raw ingest, coaching notes) → auto commit + push the personal repo (`git -C <DATA_ROOT> add -A && git -C <DATA_ROOT> commit -m "..." && git -C <DATA_ROOT> push`). No approval needed for wiki-only changes. Code changes in the public repo require the default OpenCode agent.
...
```
Silently skip if nothing to commit. Report the result in one line.
